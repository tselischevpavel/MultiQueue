...
class ConsumerTestMulti : public IConsumer<Key, Value>{
...
void produce(std::shared_ptr<QueueHandler<std::string>> qh, std::unique_ptr<Producer<int, std::string>> producer, int num){
...
void consume(std::promise<bool>&& res, ConsumerTestMulti<int, std::string>* consumer){
...
bool TestMultiPolicy::test_overflow(){
    ...
    ConsumerTestMulti<int, std::string>* consumer1 = new ConsumerTestMulti<int, std::string>();
    ...
    consumer_thread = std::thread(consume, std::move(res), consumer1);
    std::this_thread::sleep_for(std::chrono::milliseconds(2000));
    for( int i=0; i<10; ++i ) {
        producers_tread[i] = std::thread(produce, qh, std::make_unique<Producer<int, std::string>>(), i+1);
    }
